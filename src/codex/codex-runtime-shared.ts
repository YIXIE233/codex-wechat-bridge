...
export {
  ...
  getSharedSessionIdFromRuntimeState,
...
export function buildCliEnvironment(
  ...
  // as OPENAI_API_KEY, which broke Codex even though the same CLI worked
  // when launched manually from the user's shell.
...
export function buildSpawnDiagnostic(
  ...
  const lines: string[] = [
    `Failed to start Codex process: ${target}\nError: ${errorMessage}`,
    "\nPossible fixes:",
  ];
  ...
  if (errorMessage.includes("posix_spawnp") || errorMessage.includes("node-pty")) {
    ...
    if (platform === "linux") {
      lines.push(
        "- Linux requires build tools for node-pty: sudo apt install build-essential python3\n- Then reinstall: npm install -g codex-wechat-bridge@latest",
      );
    }
  } else if (errorMessage.includes("ENOENT") || errorMessage.includes("spawn")) {
    lines.push(`- The command "${target}" was not found on PATH.\n- Verify Codex is installed and accessible from your terminal.`);
  } else {
    lines.push("- Reinstall: npm install -g codex-wechat-bridge@latest");
  }

  lines.push("- Ensure Node.js >= 24.0.0: node --version");
  if (platform === "win32") {
    lines.push(
      "- Ensure Windows 10 build 18309+ (run 'winver' to check)\n- Install Visual C++ Redistributable: https://aka.ms/vs/17/release/vc_redist.x64.exe\n- Run: npm rebuild node-pty\n- Try running as Administrator",
    );
  }
