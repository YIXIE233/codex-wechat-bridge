...
import * as shared from "./codex-runtime-shared.ts";
...
export * from "./codex-runtime-shared.ts";

type CodexRuntimeOptions = shared.CodexRuntimeOptions;
...
export class CodexPtyRuntime implements CodexRuntime {
  ...
  private pty: IPty | shared.PtyLike | null = null;
  ...
  async start(): Promise<void> {
    ...
    if (this.isHeadlessRuntimeMode()) {
      this.setStatus("starting", `Starting Codex runtime host...`);
    }
    ...
    try {
      if (this.isNativePanelMode()) {
        ...
        this.afterStart();
        this.setStatus("idle", `${this.options.kind} runtime is ready.`);
      } else {
        await this.startPtyRuntime();
      }
    } catch (err) {
  ...
  async sendInput(text: string): Promise<void> {
    ...
    if (!this.pty) {
      throw new Error("Codex runtime is not running.");
    }
  ...
  async resolveApproval(action: "confirm" | "deny"): Promise<boolean> {
    ...
    return await this.resolvePtyApproval(action);
  }
  ...
  async resolveAllApprovals(action: "confirm" | "deny"): Promise<number> {
    ...
    const ok = await this.resolvePtyApproval(action);
    return ok ? 1 : 0;
  }
  ...
  getState(): CodexRuntimeState {
    return JSON.parse(JSON.stringify(this.state)) as CodexRuntimeState;
  }

  private async startPtyRuntime(): Promise<void> {
    if (this.pty) {
      return;
    }

    this.setStatus("starting", `Starting Codex runtime...`);
  ...
  private async resolvePtyApproval(action: "confirm" | "deny"): Promise<boolean> {
    if (!this.pendingApproval) {
      return false;
    }
  ...
  private async disposePtyRuntime(): Promise<void> {
    this.clearCompletionTimer();
    this.pendingApproval = null;
    this.state.pendingApproval = null;
  ...
  private defaultCompletionDelayMs(): number {
    return 5_000;
  }

  private scheduleTaskComplete(delayMs: number): void {
  ...
  private handlePtyExit(exitCode: number | undefined): void {
    this.clearCompletionTimer();
  ...
  private handleData(rawText: string): void {
...
export function shouldTreatCodexNativeExitAsExpected(params: {
  renderMode?: CodexRuntimeOptions["renderMode"];
